...
export function buildPackageJsonKeyOrderConfig(options: PackageJsonKeyOrderConfigOptions = {}): ConfigArrayValue {
  ...
  if (jsonPlugin === undefined) {
    if (options.enabled === true) {
      throw new Error(
        `@exadev/eslint-config: package.json key ordering was explicitly requested but '@eslint/json' could not be resolved. Install it with: ${JSON_PLUGIN_INSTALL_COMMAND}`,
      );
    }
    return [];
  }
