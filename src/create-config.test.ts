...
describe('exadevConfig', () => {
  it('with every optional feature forced off, returns exactly the base recommendedTypeChecked plus jsdocAndTsdoc plus jsonCanonicalConfig length, regardless of what is installed', () => {
    expect(exadevConfig({ react: false, nextjs: false, packageJsonKeyOrder: false })).toHaveLength(recommendedTypeChecked.length + jsdocAndTsdoc.length + jsonCanonicalConfig.length);
  });

  it('with no args, auto-detects against this repo\'s own real devDependencies (react + hooks + a11y + nextjs all installed for testing, no syncpack config present)', () => {
    ...
    expect(result).toHaveLength(
      recommendedTypeChecked.length + jsdocAndTsdoc.length + jsonCanonicalConfig.length + REACT_FAMILY_BLOCK_COUNT + NEXTJS_BLOCK_COUNT + PACKAGE_JSON_KEY_ORDER_BLOCK_COUNT,
    );
  });
