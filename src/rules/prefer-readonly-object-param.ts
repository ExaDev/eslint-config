...
function isFlatObjectType(checker: ts.TypeChecker, type: ts.Type, location: ts.Node): boolean {
  // A candidate must be a genuinely concrete object type at all -- see the file header for the real downstream defect this fixes (an `unknown`-typed parameter wrongly wrapped in `Readonly<...>`) and why `ts.TypeFlags.Object` is the right general gate for it, rather than a name-based exclusion list.
  if ((type.flags & ts.TypeFlags.Object) === 0) return false;
  // Checked via `.flags` rather than the `isUnion()`/`isIntersection()`/`isTypeParameter()` predicate methods: `TypeParameter extends InstantiableType extends Type {}` adds no members of its own in typescript's own .d.ts, so `Type` is structurally assignable to `TypeParameter` and negating `type.isTypeParameter()` narrows `type` to `never` (confirmed directly with a minimal tsc repro) -- a genuine quirk of that predicate's declared shape, not something specific to this file. (None of these three ever carries `TypeFlags.Object` either, so the gate above already excludes them too -- kept here regardless as a self-documenting, defence-in-depth guard rather than relying on that overlap implicitly.)
