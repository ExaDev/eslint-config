...
//
// Detection therefore needs real type information, not syntax alone (`ESLintUtils.getParserServices` + `checker.getTypeAtLocation` on the parameter's own declaration node, matching this package's other type-aware rules such as no-map-instanceof-mutation.ts). A candidate parameter is syntactically a `TSTypeLiteral` (an inline `{ ... }` shape) or a `TSTypeReference` to a plain identifier that isn't already named `Readonly` (a qualified name like `Foo.Bar` is conservatively skipped, since the referenced shape can't be reasoned about the same way). The resolved `ts.Type` is then rejected outright — treated as out of scope for THIS rule, not reported at all — when: it doesn't carry `ts.TypeFlags.Object` at all, i.e. it isn't a genuinely concrete object type (see the dedicated paragraph below for why this check exists and what it was missing); it's callable as a whole (`checker.getSignaturesOfType(type, ts.SignatureKind.Call).length > 0` — a function value as the entire parameter, not a property, is a different shape); its symbol carries `ts.SymbolFlags.Class` (a genuine user-declared `class`, whose methods can mutate its own private state in ways a property-by-property flatness check can't see); its symbol is one of `Date`/`Map`/`ReadonlyMap`/`Set`/`ReadonlySet`/`WeakMap`/`WeakSet`, matched by name rather than by the Class flag (each of these is declared in lib.d.ts as a plain ambient `interface`, never a real `class`, so none of them carry `SymbolFlags.Class` at all — confirmed as a real gap for `Date` specifically: its own methods are all pure-callable by this rule's definition and it has no writable data property, so the property loop below vacuously "passed" it as flat before this exclusion was added, even though `Readonly<Date>` protects nothing, since `readonlyDate.setFullYear(...)` still compiles and still mutates the underlying value); or it's a union, intersection, or unconstrained type parameter (each can have a shifting or absent property set that `checker.getPropertiesOfType` would report on unsafely — e.g. a union's "properties" are only the ones common to every constituent, which can vacuously read as empty and therefore falsely "flat" — none of these three ever carries `TypeFlags.Object` either, confirmed empirically, so the Object gate above already excludes them too).
//
// The `ts.TypeFlags.Object` gate was added after a real, downstream defect, not a theoretical one: a consumer package (json-operators) had a function parameter typed `EvaluationContext`, where `type EvaluationContext = unknown`, and this rule wrapped it in `Readonly<EvaluationContext>`. TypeScript's own mapped-type machinery collapses `Readonly<unknown>` down to `{}` (mapping zero keys over a non-object type yields the empty object type), and `{}` rejects `undefined` as an assignable value — so the "fix" silently narrowed a type explicitly allowed to be `undefined` into one that isn't, a genuine type-soundness bug, not a false positive to merely silence. Root cause: `unknown` (like `any`, `never`, and every bare primitive keyword type — `string`, `number`, `boolean`, `bigint`, `symbol`) carries none of the object/array/tuple/callable/class/Map/Set flags this rule already excludes, so it fell all the way through to the property/index-signature loop, where `checker.getPropertiesOfType`/`getIndexInfosOfType` both report zero entries for it — not because it has no mutable state to protect, but because it isn't an object type at all. The loop's "zero entries, nothing failed the flatness check" therefore vacuously read as "flat", when the correct reading is "not a checkable object shape in the first place". Confirmed directly (a minimal tsc repro against each) that `unknown`/`any`/`never`/every primitive keyword type all lack `ts.TypeFlags.Object`, while every genuine object/interface/type-literal/array/tuple/function/Map/Set/class type this file already special-cases DOES carry it — so gating on that flag fixes the general "is this genuinely a concrete object shape" question in one check, rather than a name-based exclusion list covering only `unknown`/`any`. (Confirmed too, empirically, that a union/intersection/type-parameter type never carries `TypeFlags.Object` either, so this new gate subsumes the union/intersection/type-parameter exclusion above — that check is kept regardless, as a self-documenting, defence-in-depth guard rather than relying on the overlap implicitly.)
//
...
interface SymbolBearing {
  getSymbol: () => { readonly flags: number } | undefined;
}

// `Type.getSymbol()` is genuinely `ts.Symbol | undefined` — unlike the deprecated, lying `Type.symbol` field (declared non-optional even though a type without an associated symbol has none at runtime) — so this reads it defensively. Exported so that "no symbol at all" path is checked directly, against a deliberately symbol-less fake type, since no real object-flagged type this rule has been run against in practice has ever been found to lack one.
export function hasSymbolFlag(type: Readonly<SymbolBearing>, flag: number): boolean {
  return ((type.getSymbol()?.flags ?? 0) & flag) !== 0;
}

// The whole resolved object type is "flat" only when it is a plain object/interface/type-alias shape (not callable or constructable as a whole, not a Map/Set, not a class instance, not a union/intersection/unconstrained type parameter — see the file header for why each of these is excluded rather than risked; an array/tuple is excluded too, but needs no dedicated check of its own — see the comment below) AND every one of its own properties and index-signature values is individually flat.
function isFlatObjectType(checker: ts.TypeChecker, type: ts.Type, location: ts.Node): boolean {
  // A candidate must be a genuinely concrete object type at all — see the file header for the real downstream defect this fixes (an `unknown`-typed parameter wrongly wrapped in `Readonly<...>`) and why `ts.TypeFlags.Object` is the right general gate for it, rather than a name-based exclusion list. Confirmed empirically that this alone already excludes a union, an intersection, and an unconstrained type parameter too (none of the three ever carries `TypeFlags.Object`), so no separate check for any of them is needed on top of it.
  if ((type.flags & ts.TypeFlags.Object) === 0) return false;
  // No separate array/tuple exclusion is needed here (prefer-readonly-array-param.ts's own domain): confirmed empirically that a real Array/ReadonlyArray/tuple type's own `[Symbol.unscopables]` property already fails the property loop below on its own for every such type, since that property's value type is never itself flat — so an explicit `checker.isArrayType`/`isTupleType` check here would only ever short-circuit to the same `false` the loop already reaches.
  ...
  if (hasSymbolFlag(type, ts.SymbolFlags.Class)) return false;

  // `Date`, like `Map`/`Set` below, is declared in lib.d.ts as a plain `interface` (`declare var Date: DateConstructor`), never a real `class` — so the `SymbolFlags.Class` check above never catches it, and it has to be matched by name here instead. Confirmed as a genuine gap, not a hypothetical one: `Date`'s own methods (`getTime`, `toISOString`, `setFullYear`, ...) are all pure-callable by this rule's own definition (no properties/index signature of their own), and it carries no writable DATA property at all, so the property loop below vacuously "passes" it as flat — but `Readonly<Date>` doesn't remove or guard any of those methods, so `readonlyDate.setFullYear(2000)` still compiles and still mutates the underlying Date, exactly the false sense of protection the Map/Set exclusion already exists to prevent. `WeakMap`/`WeakSet` share the identical profile (hidden mutable state exposed only through methods a shallow wrapper can't touch) and are excluded for the same reason, not because either was independently confirmed to reach this code path.
