  ...
  create(context) {
    ...
    return {
      MemberExpression(node: TSESTree.MemberExpression) {
        ...
        // A VariableDeclarator's own `init` field is the only place a value expression can sit — its `id` is a binding target (an Identifier or a destructuring pattern), never this MemberExpression — so once `node.parent` is confirmed to be a VariableDeclarator, `parent.init` is necessarily this same node; there is no need to compare the two.
