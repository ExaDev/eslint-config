...
// Extracts and validates the `mode` option. A standalone function (not inline in create) so ESLint's `any`-typed `context.options[0]` is funneled through an `unknown` parameter boundary — passing `any` into `unknown` is safe, whereas inline member access on `any` (`options.mode`) propagates `any` through every later use and trips the type-aware lint rules. Inside the function `options` is `unknown`, so the narrowing composes cleanly without an assertion. Exported so its own throw — a safety net behind the rule's schema, which ESLint's own Linter/RuleTester already enforce before create() is ever called with invalid options — can be exercised directly rather than left permanently unreachable through real linting.
export function readMode(options: unknown): BarrelMode {
  // `undefined` already fails `typeof options !== 'object'` (typeof undefined is 'undefined', never 'object'), so a separate `options === undefined` check ahead of it can never be the deciding disjunct — it would only ever agree with the check right after it.
  if (typeof options !== 'object' || options === null || !('mode' in options) || !isBarrelMode(options.mode)) {
    ...
    return {
      ...
      ExportNamedDeclaration(node) {
        ...
        if (hasSource(node) && !isInsideAmbientModuleDeclaration(node)) {
          ...
          } else if (mode === 'siblings' && !isDirectSibling(source)) {
        ...
        for (const violation of detector.violations()) {
          if (isPermittedBarrel(filename, mode)) {
            ...
            if (mode === 'siblings') {
              ...
              if (!isDirectSibling(importSource)) {
