  ...
  create(context) {
    ...
    function isGuardedByArrayIsArray(
      ...
      while (current.parent) {
        ...
        // No separate `parent.right === current` check is needed here: current only ever climbs up from the mutating call's own ancestor chain, so for it to equal parent.left while matchesArrayIsArrayOn(parent.left, ...) is also true, current would have to BE the separate Array.isArray(...) guard expression itself -- structurally impossible, since current and the guard are always two different nodes in the tree.
        if (
