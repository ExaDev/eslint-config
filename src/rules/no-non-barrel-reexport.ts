...
interface DeclaredVariableLike {
  readonly defs: readonly { readonly node: SyntaxElement }[];
  // A hole at a given index is never something a real scope-manager Variable's own references array actually contains, but array indexing itself can never statically rule it out — modelled explicitly here (rather than narrowed away) so the function's own defensive check against it is a real, reachable branch, not a TypeScript-only artifact.
  readonly references: readonly ({ readonly identifier: ReferenceIdentifier } | undefined)[];
}
// Only ever needs to look up the declared variables for one node — narrower than the full SourceCode interface, so a test fixture can satisfy this shape directly with a plain object literal.
interface DeclaredVariableLookup {
