    ...
    return {
      CallExpression(node) {
        ...
        if (target?.type === AST_NODE_TYPES.ObjectExpression && sourcesAreSpreadable) {
          // A leading `{` is ambiguous at the start of an ExpressionStatement (parsed as a block, not an object literal) — wrap in parens exactly when this call is the statement's entire expression. An ExpressionStatement has exactly one child (its own `expression`), so once `node.parent` is confirmed to be one, `node.parent.expression` is necessarily this same node — there is no need to compare the two.
          const isBareStatement = node.parent.type === AST_NODE_TYPES.ExpressionStatement;
          context.report({
            node,
            messageId: 'unsound',
            fix(fixer) {
        ...
        // Case B: an existing identifier, mutated as a bare statement — offer a suggestion only, and only when the binding is genuinely reassignable. As above, an ExpressionStatement parent's own `expression` is necessarily this same node, so that comparison is redundant once the parent's type is confirmed.
        if (target?.type === AST_NODE_TYPES.Identifier && sourcesAreSpreadable && node.parent.type === AST_NODE_TYPES.ExpressionStatement) {
