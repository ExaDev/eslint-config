...
const recommendedTypeChecked: ConfigArrayValue = [
  ...
  ...scopeToJsTs(tseslint.configs.stylisticTypeChecked),
  {
    ...
    rules: {
      ...
      // `ignore: [-1, 0, 1, 2]` covers the handful of universally-idiomatic bare numbers (loop bounds, array-length-minus-one, binary toggles) a named constant wouldn't clarify; `ignoreArrayIndexes: true` since `array[0]`/`array[2]` isn't a "magic number" smell; `ignoreEnums: true` since an enum member's own numeric value isn't magic at its declaration; `ignoreReadonlyClassProperties: true` since a readonly class property IS already a named constant, matching this config's own named-constant carve-out; `ignoreDefaultValues: true` since a default parameter value is self-documenting at the call site; `ignoreNumericLiteralTypes: true` since a numeric literal used as a TYPE (`type Indent = 2 | 4`) is the literal type itself, not a value that could be replaced by a named constant without losing the type. `detectObjects` is left at the rule's own default (`false`): numeric object-property VALUES in plain data/config objects are deliberately not flagged — exhaustively naming every literal in a data structure would be absurd, so this is a deliberate choice, not a gap.
